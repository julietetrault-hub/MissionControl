...
// The issue is likely CSS or element nesting hiding the content. 
// I will move the metrics to be outside the summary-card div, 
// so they are top-level sections in the Financials tab.
...
// 1. Clean up ALL previous injections and duplicates
function purge(content) {
    let result = content;
    const targets = [
        /<div class="section" id="jj-metrics">[\s\S]*?</div>/g,
        /<div class="section" id="overall-metrics">[\s\S]*?</div>/g,
        /<div class="section" id="expense-metrics">[\s\S]*?</div>/g,
        /<div class="section">\s*<h3>Overall Business: Top 5 Financial Metrics[\s\S]*?</div>/g,
        /<div class="section">\s*<h3>J&J Modern Luxury: Top 5 Performance Metrics[\s\S]*?</div>/g,
        /<div class="section">\s*<h3>Top 5 Expense Optimization Metrics[\s\S]*?</div>/g,
        /<div class="section">\s*<h3>Top 5 Business Metrics[\s\S]*?</div>/g
    ];
    targets.forEach(t => { result = result.replace(t, ''); });
    return result;
}

html = purge(html);

// 2. Inject JJ Metrics into the J&J Tab
const jjTabHeader = '<h3>Luxury Performance Metrics</h3>';
html = html.replace(jjTabHeader, jjTabHeader + jjMetrics);

// 3. Inject Overall and Expense metrics into the Financials tab
// We'll place them after the summary-cards container ends
const summaryCardsMarker = '<div class="summary-cards">';
let searchPos = html.indexOf(summaryCardsMarker);
// Find the closing </div> of the summary-cards grid
let openDivs = 1;
let currentPos = searchPos + summaryCardsMarker.length;
...
const insertionPoint = currentPos;
html = html.substring(0, insertionPoint) + overallMetrics + expenseMetrics + html.substring(insertionPoint);
...
console.log('✅ Mission Control layout fixed and updated.');
